...
def align_and_rotate_star(
    ...
    cutout_mask: Optional[np.ndarray],
    ...
    cutout_mask : np.ndarray or None
        mask bad values map.
    shift : Tuple[float, float]
    ...
    reduced_wht = np.ones_like(reduced_image)
    ...
    if cutout_mask is not None and np.sum(cutout_mask)>0:

        cutout_mask_dilated = binary_dilation(cutout_mask,np.ones((order+1,order+1)))
        rotated_mask = process_cutout(
            data=cutout_mask_dilated,
            shift=shift,
            angle=rot_angle,
            scale_factor=upsample_ratio,
            output_shape=output_shape,
        ...
        reduced_mask = block_reduce(rotated_mask, high_res_factor, func=np.max).astype(bool)

        reduced_wht[reduced_mask] = 0
        reduced_image[reduced_mask] = 0
...
def get_drizzle_transform(shape_in, shape_out, shift, angle_deg, scale_factor):
    """
    Computes the FORWARD transform parameters required by the drizzle package.
    Maps INPUT (low-res) coordinates -> OUTPUT (high-res, rotated) coordinates.

    This is the mathematical INVERSE of ``get_combined_transform`` (which
    goes output->input), so it must use ``scale_factor`` DIRECTLY, not its
    reciprocal — using ``1/scale_factor`` here (as a previous version of
    this file did) double-inverts the magnification applied downstream by
    ``drizzle_resample_star`` (which itself already converts
    ``star.scale_factor`` -> the correct magnification before calling
    here). That double inversion is what was producing an under-magnified
    (too-small) drizzled star.

    Forward operations (on image):
        1. Upscale by scale_factor (center preserved)
        2. Shift by 'shift' in upscaled, UN-ROTATED coordinates
        3. Rotate by angle_deg around the output center

    Parameters
    ----------
    shape_in : tuple
        Input image shape (h, w) in NumPy (y, x) order
    shape_out : tuple
        Output image shape (h, w) in NumPy (y, x) order
    shift : tuple
        (shift_y, shift_x) in HIGH-RES (output-grid) pixels, BEFORE
        rotation. See note in ``get_combined_transform``.
    angle_deg : float
        Rotation angle in degrees (counter-clockwise)
    scale_factor : float
        Magnification: output pixels per input pixel (same convention as
        ``get_combined_transform``'s ``scale_factor``, NOT its inverse).

    Returns
    -------
    matrix : 2x2 array
        The Jacobian matrix mapping dx_in, dy_in -> dx_out, dy_out
    xoffset, yoffset : float
        The coordinate offsets expected by drizzle.add_image()
    """
    # 1. Flip NumPy (y, x) shapes/shifts into standard geometric (x, y) order
    ny_in, nx_in = shape_in
    ny_out, nx_out = shape_out
    dy_high, dx_high = shift

    # Centers in geometric (x, y) coordinates
    center_in = np.array([(nx_in - 1) / 2.0, (ny_in - 1) / 2.0])
    center_out = np.array([(nx_out - 1) / 2.0, (ny_out - 1) / 2.0])
    shift_xy = np.array([dx_high, dy_high])

    # 2. Define forward rotation matrix (R) around (x, y)
    theta = np.deg2rad(angle_deg)
    c, s = np.cos(theta), np.sin(theta)
    rot_forward = np.array([[c, -s],
                             [s, c]])

    # Forward scaling matrix (S) -- FIXED: direct scale_factor, not 1/scale_factor
    scale_forward = np.array([[scale_factor, 0],
                               [0, scale_factor]])

    # Combined Forward Linear Matrix: J = R @ S
    matrix = rot_forward @ scale_forward

    # 3. Derive the forward offset mathematically
    # x_out = R @ ( S @ x_in + shift_xy - center_out ) + center_out
    #       = (R @ S) @ x_in + R @ (shift_xy - center_out) + center_out
    # => offset = R @ (shift_xy - center_out) + center_out

    offset_xy = rot_forward @ (shift_xy - scale_forward @ center_in) + center_out
...
# DRIZZLE-BASED SINGLE-STAR RESAMPLING
def build_pixmap(shape_in, shape_out, shift, angle_deg, scale_factor):
    """
    Build the (Ny_in, Nx_in, 2) pixmap that `drizzle.resample.Drizzle.add_image`
    expects: for every INPUT pixel, the (x, y) coordinate it maps to in the
    OUTPUT frame, under the same "upscale -> shift -> rotate" forward
    convention used throughout this module.

    Parameters
    ----------
    shape_in, shape_out : tuple
        (Ny, Nx) shapes, NumPy order.
    shift : tuple
        (shift_y, shift_x) in OUTPUT-grid pixels, before rotation.
    angle_deg : float
        Forward rotation angle in degrees.
    scale_factor : float
        Magnification: output pixels per input pixel.

    Returns
    -------
    pixmap : (Ny_in, Nx_in, 2) float64 array
        pixmap[..., 0] = output X coordinate, pixmap[..., 1] = output Y
        coordinate, for each input pixel.
    """
    matrix, xoffset, yoffset = get_drizzle_transform(
        shape_in=shape_in,
        shape_out=shape_out,
        shift=shift,
        angle_deg=angle_deg,
    ...
    ny_in, nx_in = shape_in
    yy, xx = np.mgrid[0:ny_in, 0:nx_in]
    xx = xx.astype(np.float64)
    yy = yy.astype(np.float64)

    out_x = matrix[0, 0] * xx + matrix[0, 1] * yy + xoffset
    out_y = matrix[1, 0] * xx + matrix[1, 1] * yy + yoffset

    pixmap = np.stack([out_x, out_y], axis=-1)
    return pixmap
...
class StarContainer:
    ...
    cutout_mask : np.ndarray, optional
        2D array containing the masked bad values.
    ...
    cutout_mask: Optional[np.ndarray] = None

    #contained the shifted (polynomial) arrays for visualization in the core package
    cutout_elaborated: Optional[np.ndarray] = None
    cutout_wht_elaborated: Optional[np.ndarray] = None

    #used only by the builder
    cutout_elaborated_builder: Optional[np.ndarray] = None
    cutout_wht_elaborated_builder: Optional[np.ndarray] = None

    #same size and shift as cutout, cutout_wht, cutout_mask. 
    #containes the masked weight after the sigma rejection. Filled by the builder
    cutout_weights_rej_builder: Optional[np.ndarray] = None
    ...
    def __repr__(self) -> str:
        ...
        pa_str = f"{self.best_pa:.1f}°" if self.best_pa is not None else "None"
        ...
        return (
            ...
            f"  PA={pa_str}°, rotation={rot_str}, shift={shift_str},\n"
            f"  status={status}\n"
            f")"
        )
...
class StarCutouts:
    ...
    final_resolution : float, optional
        Target pixel scale in arcseconds/pixel for the final ePSF.
        If None, it's the resolution of the lowest res image multiplied by the upsampling factor
    final_size : tuple or np.ndarray, optional
        Size of the final cutout in pixels in not upsampled space (height, width). Default is (205, 205).
    ...
    def __init__(self,
        ...
        if final_resolution is None:
            images_resolutions = [image_collection.data[image_collection.image_names[j]].pixel_scale.value for j in range(image_collection.n_images)]
            final_resolution =  np.max(images_resolutions)/ upsample_factor  # arcsec/pixel

        print(f"The ePSF will be computed at the ePSF final resolution of {final_resolution} \"/px")


        # Store image collection and resolution parameters
        self.image_collection = image_collection
        self.final_resolution = final_resolution
        self.cutout_shape = np.asarray(cutout_shape)
        ...
        for image_name in self.image_names:
            ...
            elif abs(1/scale_factor - round(1/scale_factor)) < 1e-6:
                self.scale_factors[image_name] = 1/round(1/scale_factor)
            else:
                raise ValueError(
    ...
    def _load_catalogue(self) -> bool:
        ...
        Updates the stars list with is_valid, and mask values
        from the cached file if it exists. Do not load rotation and mask, forcing these values to be recomputed.
        ...
        try:
            ...
            print(f"Loaded {n_matched}/{len(self.master_catalogue)} stars from saved catalogue")
            return True
    ...
    def extract_cutout_ith_star(self,
                                 ...
                                 extract_invalid_star = False) -> StarContainer:
        """
        ...
        extract_invalid_star : bool, optional
            Extract the star cutout even if it's marked as not valid
        ...
        if self.stars[row_index] is not None and self.stars[row_index].is_valid==0 and not extract_invalid_star:
            #The star is invalid and do not need to be extracted
            return self.stars[row_index]
        ...
        x_star = self.master_catalogue['X_IMAGE'][row_index]-1# sectractor use 1-indexing
        y_star = self.master_catalogue['Y_IMAGE'][row_index]-1# sectractor use 1-indexing
        ...
        cutout_shape = tuple((self.cutout_shape * self.scale_factors[image_name] * self.upsample_factor).astype(int))
        ...
        bad_star = False
        if not cutout_shape == cutout.shape:
            bad_star = True

        cutout_mask = (cutout==0) | ~np.isfinite(cutout)
        ...
        if self.stars[row_index] is None or force_extraction:
            ...
            star.cutout_mask = cutout_mask
            star.scale_factor = self.scale_factors[image_name]
            star.best_pa = image_obj.best_PA

        if bad_star:
            star.mask = True
            star.failure_reason = f"The star cutout {cutout.shape} is smaller than requested {cutout_shape}"
    ...
    def extract_all_cutouts(self,
                            magnitude_field: str = "MAG_APER_1",
                            force_extraction = False,
                            extract_invalid_stars = False) -> None:
        """
        ...
        for i in tqdm(range(n_stars)):
            try:
                self.extract_cutout_ith_star(i, magnitude_field, force_extraction=force_extraction, extract_invalid_star = extract_invalid_stars)
            except Exception as e:
    ...
    def subtract_background(self,
        ...
        try:
            mean_val, median_val, std_val = sigma_clipped_stats(
                star.cutout,
                mask = self.stars[row_index].cutout_mask,
                sigma=sigma,
                maxiters=maxiters
            )
        except:
    ...
    def subtract_background_all(self,
        ...
        for i in tqdm(range(n_stars)):
            if self.stars[i] is None:
                raise ValueError(f"Star {i} has not been extracted.")

            if self.stars[i].cutout is not None:
                self.subtract_background(i, sigma, maxiters, method)
            elif self.stars[i].is_valid != 0:
                raise ValueError(f"Star {i} is not invalid, but doesn't have its cutout extracted")
    ...
    def compute_shift_gaussian(self,
        ...
        if star.cutout is None and star.is_valid==0:
            return star.shift
        
        scale_factor = star.scale_factor

        if centroid_radius is None:
            centroid_radius = int(self.DEFAULT_CENTROID_RADIUS * scale_factor * self.upsample_factor)
        if max_shift is None:
            max_shift = self.DEFAULT_MAX_SHIFT * scale_factor * self.upsample_factor
        ...
        try:
            ...
            plt.imshow(star.cutout,norm=AsinhNorm(1e-5))
            print(c_data_center)
            plt.show()


        data = star.cutout[slice_d]
        mask_data = star.cutout_mask[slice_d]
        ...
        try:
            with warnings.catch_warnings():
                ...
                x0, y0 = centroid_2dg(data, error=error, mask=mask_data)
        except:
            star.shift = (0,0)
            return (0,0)
    ...
    def compute_rotation(self,
        ...
        if ignore_masked_stars and (star.is_valid == 0 or star.mask):
            return None
        ...
        if rotate_first_by_best_pa:
            elaborated_star = process_cutout(star.cutout,star.shift,-star.best_pa,order=1)

        elif star.shift is not None:
            elaborated_star = process_cutout(star.cutout,star.shift,0.0,order=1)

        else:
            if verbose:
                print(f"Warning: no shift computed for star {row_index}")
            elaborated_star = star.cutout
    ...
    def _plot_polar_conversion(self,
        ...
        # Original PSF
        im = ax1.imshow(psf_norm, origin="lower", norm=AsinhNorm(data_norm_scale/scale_factor**2/self.upsample_factor**2, 0), cmap="magma")
        ax1.set_xlabel("x [px]")
        ax1.set_ylabel("y [px]")
        ax1.set_title("PSF")
        ...
        im = ax2.imshow(polar_psf, origin="lower", norm=AsinhNorm(data_norm_scale/scale_factor**2/self.upsample_factor**2, 0),
    ...
    def mask_star_bad_rotation(self,
        ...
        if star.is_valid == 0 or star.mask:
            return False
    ...
    def align_and_rotate_star(self,
                            ...
                            fine_rotation: bool = False,
                            ignore_masked_stars: bool = True,
                            order: int = 1) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        ...
        reduced_image, reduced_wht = align_and_rotate_star(
            ...
            final_size=self.cutout_shape * self.upsample_factor,
