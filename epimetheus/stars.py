...
class StarSelector:
    ...
    def __init__(self, 
        ...
        os.makedirs(os.path.join(cache_dir, "stars"),exist_ok=True)

        self.cache_dir = cache_dir
        
        # Extract relevant attributes from Image
        self.image_path = os.path.join(cache_dir, f"stars/tmp_img.fits")
        ...
        if self.use_variance:
            self.weight_path = os.path.join(cache_dir, f"stars/tmp_weight.fits")
    ...
    """def _clean_data(self, img, replace_val=-1e30):
        out = np.array(img, copy=True)
        
        invalid_mask = (out == 0)
        invalid_mask |= ~np.isfinite(out)
        
        out[invalid_mask] = replace_val
        return out"""
    ...
    def _clean_data(self, img, replace_val=-1e30):
        ...
        for i in tqdm(range(0, img_flat.size, chunk_size)):
            chunk_in = img_flat[i : i + chunk_size]
            chunk_out = out_flat[i : i + chunk_size]
            
            chunk_out[:] = chunk_in
            
            mask = (chunk_out == 0)
            mask |= ~np.isfinite(chunk_out)
            
            chunk_out[mask] = replace_val
    ...
    def _load_image(self):
        print("Creating clean copy of image")
        fits.PrimaryHDU(
            data=self._clean_data(self.image.image_data, replace_val=-1e30), 
            header=self.image.image_header
        ).writeto(self.image_path, overwrite=True)
        
        if self.use_variance:
            print(f"Creating clean copy of {self.image.variance_type} image")
            replace_val = 0.0 if self.image.variance_type == "weight" else 1e30

            fits.PrimaryHDU(
                data=self._clean_data(self.image.var_data, replace_val=replace_val), 
                header=self.image.var_header
            ).writeto(self.weight_path, overwrite=True)
    ...
    def run_sextractor(self, force_rerun: bool = False) -> Table:
        ...
        print("Calling Sextractor")
        
        # Create SEW object and run
        sew = sewpy.SEW(sexpath=self.sexpath, params=params, config=config)
        
        try:
            print(f"Running SExtractor on {self.image.image_path}...")
            if self.use_variance:
                print(f"  Using weight image: {self.image.variance_path} ({self.weight_type})")
